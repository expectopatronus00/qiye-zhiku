...
- **进阶能力（v1.6）**：知识图谱（上传文档自动抽取实体与共现关系建图——jieba 词性 + 内置信创领域词典（昇腾/寒武纪/摩尔线程/海光/飞腾/鲲鹏/龙芯/兆芯/昆仑/麒麟/Atlas/MLU/MUSA/DCU 等 40+ 词，`graph.entity_dict` 可扩展），SQLite 存储按库隔离，`/api/graph/*` 查询实体/关系/统计，侧边栏图谱面板（实体标签云 + 点击展开关联关系），图谱问答增强——问题实体命中库内实体时用实体名向量检索注入补充上下文，回答带 `entity_hits` 标签并与缓存/对话历史联动持久化）、多模态 VLM 图表理解（OpenAI 兼容端点对接昇腾 CANN(vLLM-Ascend)/寒武纪部署的 Qwen2.5-VL 等信创 VLM，OCR 文本过少视为图表自动生成中文描述入库，未配置 VLM 自动降级 RapidOCR 纯文本不阻塞）、Webhook 通知（飞书/钉钉自定义机器人，逗号分隔多 URL，事件：文档上传完成/后台任务失败/安全告警/用户反馈，后台线程发送 + 5s 超时 + 单次重试，失败仅记日志不阻塞主流程，管理台可热更新 URL）
- **性能与高可用（v1.5）**：异步任务队列（`app/core/tasks.py` TaskManager：SQLite 持久化 + 后台 worker 线程池 + 处理器注册表，上传文档超过 `document.async_upload_threshold`（默认 5MB）自动转后台索引、接口立即返回 `task_id`，`GET /api/tasks` 查询/过滤，前端轮询展示"后台处理中"）、Prometheus 指标（`GET /metrics` 手写文本格式零依赖：QPS 请求计数 + 请求/检索/LLM 延迟直方图，业务埋点自动采集）、热门问题缓存（LRU 256 + TTL 1h，问题空白/大小写标准化，纯 RAG 新会话命中响应带 `cached:true`，上传文档/删除知识库自动按库失效）、Redis 会话共享（`security.redis_url` 配置即启用，token→用户映射 + TTL 过期，Redis 故障自动回退 SQLite 双写，多副本部署会话一致）
...
| 2026-08-08 | v1.5 | 性能与高可用：异步任务队列（TaskManager：SQLite 持久化 data/tasks.db + 2 worker 线程池 + 处理器注册表 + async 处理器自动包装，上传 >5MB 自动转后台、立即返回 task_id，/api/tasks 查询/过滤，前端轮询展示处理状态）、Prometheus 指标（/metrics 手写文本格式：QPS 计数 + 请求/检索/LLM 直方图，中间件与业务埋点零依赖）、热门问题缓存（LRU 256 + TTL 1h，标准化 key，纯 RAG 新会话命中 cached:true，上传/删库按库失效）、Redis 会话共享（security.redis_url 可选配置，token→user 映射 + TTL，故障回退 SQLite 双写）、config.example.yaml 同步（async_upload_threshold/cache/redis_url）、修复上传异步响应模型序列化 500（Union 双响应），全量测试 232 项全过（新增 26 项），真机验证 21 项（异步上传 accepted→success/任务查询/metrics/缓存命中与失效） |
