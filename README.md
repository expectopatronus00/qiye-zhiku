...
- **系统管理后台（v1.1）**：管理控制台 4-Tab——审计日志（过滤/分页/CSV 导出）、用户管理（代建/禁用启用/重置密码/解锁/删除，禁用即时吊销令牌，删除用户知识库自动转移管理员）、知识库配额（块数/文档数上限，超限上传 403，-1 不限）、系统配置（LLM/检索/重排等热更新并写回 config.yaml，密钥脱敏不回显）
