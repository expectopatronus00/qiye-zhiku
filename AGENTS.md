...
面向央企 AI 场景的私有化 RAG 知识库问答系统（FastAPI + ChromaDB/Milvus + Ollama qwen2.5:7b + nomic-embed-text + bge-reranker-base）。当前版本 v1.5（性能与高可用），路线见 ROADMAP.md，迭代进度按"每日一版"节奏推进（v0.1~v1.5 已完成）。
...
app/routers/       API 层（prefix 各自带 /api）
  ...
  documents.py     文档（/api/documents，上传/列表/预览；v1.5 大文件超 async_upload_threshold 转后台任务返回 accepted+task_id）
...
tests/             pytest 测试（232 项，按模块拆分 test_*.py）
```
